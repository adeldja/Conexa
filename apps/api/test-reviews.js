  ...
  try {
    ...
    try {
      ...
      if (allReviews.data.length > 0) {
        console.log('Premier review:', JSON.stringify(allReviews.data[0], null, 2));
      }
    } catch (error) {
      console.log('Erreur lors de la récupération des reviews:', error.response?.data || error.message);
    }
    ...
    try {
      ...
      if (providers.length > 0 && clients.length > 0) {
        ...
        try {
          ...
          const providerReviews = await axios.get(`${API_BASE}/reviews/provider/${providerId}`);
          console.log('Reviews du provider:', JSON.stringify(providerReviews.data, null, 2));
          
          // 5. Récupérer les stats du provider
          console.log(`\n📈 Récupération des stats du provider ${providerId}...`);
          const providerStats = await axios.get(`${API_BASE}/reviews/provider/${providerId}/stats`);
          console.log('Stats du provider:', JSON.stringify(providerStats.data, null, 2));
          
        } catch (error) {
          console.log('Erreur lors de la création du review:', error.response?.data || error.message);
        }
      }
      
    } catch (error) {
      console.log('Erreur lors de la récupération des utilisateurs:', error.response?.data || error.message);
    }
...
async function createTestReviews() {
  try {
    ...
    const testReviews = [
      {
        rating: 5,
        comment: 'Service exceptionnel ! Je recommande vivement. Très professionnel et ponctuel.',
        providerId,
        clientId: clients[0]?.id
      },
      {
        rating: 4,
        comment: 'Très bon travail, quelques petits détails à améliorer mais dans l\'ensemble très satisfait.',
        providerId,
        clientId: clients[1]?.id || clients[0]?.id
      },
      {
        rating: 5,
        comment: 'Parfait ! Exactement ce que je cherchais. Prestataire à l\'écoute et compétent.',
        providerId,
        clientId: clients[0]?.id
      },
      {
        rating: 3,
        comment: 'Correct, sans plus. Le travail a été fait mais manquait un peu de créativité.',
        providerId,
        clientId: clients[1]?.id || clients[0]?.id
      },
      {
        rating: 5,
        comment: 'Fantastique ! Dépassé mes attentes. Communication excellente tout au long du projet.',
        providerId,
        clientId: clients[0]?.id
      }
    ];
    ...
    for (const review of testReviews) {
      if (review.clientId) {
        try {
          ...
          console.log(`✅ Review créé: ${review.rating}⭐ - "${review.comment.substring(0, 50)}..."`);
        } catch (error) {
          console.log(`❌ Erreur pour le review: ${error.response?.data?.message || error.message}`);
        }
      }
    }
    
    console.log('\n📊 Récupération des stats finales...');
    const finalStats = await axios.get(`${API_BASE}/reviews/provider/${providerId}/stats`);
    console.log('Stats finales:', JSON.stringify(finalStats.data, null, 2));
    
  } catch (error) {
    console.error('Erreur lors de la création des reviews de test:', error.message);
  }
}
