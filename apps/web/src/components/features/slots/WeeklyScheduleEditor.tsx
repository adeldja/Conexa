...
import { WeeklySchedule, DaySchedule, TimeSlot, DAYS_OF_WEEK, DEFAULT_WEEKLY_SCHEDULE } from '@/types/types';
  ...
  const updateDayEnabled = (dayKey: string, enabled: boolean) => {
    const newSchedule = {
      ...
      [dayKey]: {
        ...schedule[dayKey],
        enabled,
        timeSlots: enabled ? schedule[dayKey].timeSlots.length > 0 ? schedule[dayKey].timeSlots : [{ start: '09:00', end: '17:00' }] : []
      }
    };
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-lg font-semibold text-slate-900">Configuration des horaires hebdomadaires</h3>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  {daySchedule.enabled && (
                    <span className="text-sm text-slate-500">
                      {daySchedule.timeSlots.length} plage{daySchedule.timeSlots.length > 1 ? 's' : ''}
                    </span>
                  )}
                </div>
                <svg 
