...
export default function AdvancedSlotsManager() {
  ...
  const handleSlotStatusChange = async (slotIndex: number, status: 'available' | 'closed') => {
    ...
    try {
      ...
      await advancedSlotsService.toggleSlotAvailability(slot.backendId, status === 'available');
      ...
      setError(
        error instanceof Error ? error.message : 'Erreur lors de la modification du créneau'
      );
    }
  };
  ...
  const handleDeleteAllSlots = async () => {
    ...
    try {
      ...
      for (const slot of generatedSlots) {
        if (slot.backendId) {
          try {
            ...
            if (slot.booking) {
              failedSlots.push(
                `${slot.date} ${slot.startTime}-${slot.endTime} (réservé par ${slot.booking.clientName})`
              );
            } else {
      ...
      if (deletedCount > 0 && failedCount === 0) {
        ...
        setError(
          `${deletedCount} créneaux supprimés. ${failedCount} créneaux n'ont pas pu être supprimés car ils ont des réservations.`
        );
      } else if (failedCount > 0) {
        setError(
          `Aucun créneau supprimé. ${failedCount} créneaux ont des réservations et ne peuvent pas être supprimés.`
        );
      }
    } catch (error) {
      console.error('Erreur lors de la suppression des créneaux:', error);
      setError(
        error instanceof Error ? error.message : 'Erreur lors de la suppression des créneaux'
      );
  ...
  const handleDeleteAvailableSlots = async () => {
    ...
    if (
      !confirm(
        `Êtes-vous sûr de vouloir supprimer les ${availableSlots.length} créneaux disponibles ? Cette action est irréversible.`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      setError(
        error instanceof Error
          ? error.message
          : 'Erreur lors de la suppression des créneaux disponibles'
      );
      // Recharger en cas d'erreur pour avoir l'état correct
      await loadExistingSlots();
    } finally {
      ...
      {isLoadingSlots ? (
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-12">
          <div className="text-center">
            <svg
              className="animate-spin mx-auto h-8 w-8 text-slate-400"
              fill="none"
              viewBox="0 0 24 24"
            >
              <circle
        ...
        <SlotsCalendar slots={generatedSlots} onSlotStatusChange={handleSlotStatusChange} />
      )}
    </div>
  );
}
