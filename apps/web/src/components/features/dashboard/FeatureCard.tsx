...
export default function FeatureCard({
  ...
  const iconWrapper = (
    <div
      className={`w-12 h-12 ${isActive ? iconBgColor : 'bg-slate-100'} rounded-lg flex items-center justify-center`}
    >
      {icon}
    </div>
  );
  ...
  const button =
    isActive && href ? (
      <Link href={href}>
        <span
          className={`inline-flex items-center justify-center w-full px-4 py-2 ${buttonBgColor} ${buttonHoverColor} text-white text-sm font-medium rounded-lg transition-colors`}
        >
          {buttonText}
        </span>
      </Link>
