...
class AdvancedSlotsService {
  ...
  async getProviderSlots(providerId: string): Promise<BackendSlot[]> {
    ...
    return ApiClient.request<BackendSlot[]>(`${API_CONFIG.ENDPOINTS.AVAILABILITY}/provider/${providerId}`);
  }

  async getProviderBookings(providerId: string): Promise<BackendBooking[]> {
    try {
      return await ApiClient.request<BackendBooking[]>(`${API_CONFIG.ENDPOINTS.BOOKINGS}/provider/${providerId}`);
    } catch (error) {
  ...
  async deleteSlot(id: string): Promise<void> {
    await ApiClient.request<void>(`${API_CONFIG.ENDPOINTS.AVAILABILITY}/${id}`, { method: 'DELETE' });
  }
